...
const AddHotel = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            console.log("Form data sent:", form); //add this line
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                    ...
                    <div className="flex space-x-4">
                        <div className="w-full">
                            ...
                            <input type="date" name="AvailableFrom" onChange={handleChange} required className="border p-2 w-full" />
                        </div>
                        <div className="w-full">
                            <label htmlFor="AvailableTo" className="block text-sm font-medium mb-1">Available To</label>
                            <input type="date" name="AvailableTo" onChange={handleChange} required className="border p-2 w-full" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        {form.images.map((image, index) => (
                            <div key={index} className="flex items-center space-x-2 mb-2">
                                <input type="text" value={image} placeholder={`Image URL ${index + 1}`} onChange={(e) => handleImageChange(index, e.target.value)} className="border p-2 w-full" />
                                <button type="button" onClick={() => removeImageField(index)} className="bg-red-500 text-white py-1 px-2 rounded">Remove</button>
                            </div>
                        ))}
                        <button type="button" onClick={addImageField} className="mt-2 bg-blue-500 text-white py-2 px-4 rounded">Add Image URL</button>
                    </div>

                    <button type="submit" className="bg-orange-500 text-white py-2 px-4 rounded mt-4">Add Hotel</button>
                </form>
            </div>
        </div>
    );
